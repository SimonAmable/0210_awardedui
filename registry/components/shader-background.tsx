...
export function ShaderBackground({ mode, colors, preset = "smoke", intensity = 100, zoom = 50, warp = 50, contrast = 25, speed = 33, grain = 74, drift = 0, animate = true, reverse = false, rotation = 0, offset = defaultOffset, seed = 0, smoothBlend = false, cursor = false, cursorEffect = "velocity", cursorStrength = 50, cursorRadius = 50, className }: ShaderBackgroundProps & { mode: "smoke" | "caustics" }) {
  ...
  useEffect(() => {
    ...
    const render = (now: number) => { if (!visible || document.hidden) return; resize(); gl.uniform2f(uniform("u_resolution"), width, height); gl.uniform1f(uniform("u_time"), now * .001); gl.uniform2f(uniform("u_pointer"), pointer.x, pointer.y); gl.uniform2f(uniform("u_velocity"), pointer.velocityX, pointer.velocityY); gl.uniform3fv(uniform("u_colors"), palette); gl.uniform1f(uniform("u_style"), mode === "smoke" ? 0 : 1); gl.uniform1f(uniform("u_intensity"), intensity / 100); gl.uniform1f(uniform("u_zoom"), .5 + zoom / 50); gl.uniform1f(uniform("u_warp"), warp / 100); gl.uniform1f(uniform("u_contrast"), contrast / 100); gl.uniform1f(uniform("u_speed"), speed / 100); gl.uniform1f(uniform("u_grain"), grain / 100); gl.uniform1f(uniform("u_drift"), drift / 100); gl.uniform1f(uniform("u_animate"), shouldAnimate ? 1 : 0); gl.uniform1f(uniform("u_reverse"), reverse ? 1 : 0); gl.uniform1f(uniform("u_rotate"), rotation * Math.PI / 180); gl.uniform1f(uniform("u_seed"), seed); gl.uniform1f(uniform("u_smooth_blend"), smoothBlend ? 1 : 0); gl.uniform2f(uniform("u_offset"), offset.x / 100, offset.y / 100); gl.uniform1f(uniform("u_cursor_on"), cursor ? 1 : 0); gl.uniform1f(uniform("u_cursor_effect"), Math.max(0, effect)); gl.uniform1f(uniform("u_cursor_strength"), cursorStrength / 100); gl.uniform1f(uniform("u_cursor_radius"), .05 + cursorRadius / 100); gl.drawArrays(gl.TRIANGLES, 0, 3); pointer.velocityX *= .9; pointer.velocityY *= .9; frame = requestAnimationFrame(render) }
  ...
  }, [animate, contrast, cursor, cursorEffect, cursorRadius, cursorStrength, drift, grain, intensity, mode, offset, paletteColors, reverse, rotation, seed, smoothBlend, speed, warp, zoom])
