...
const propRows: Record<ComponentSlug, PropRow[]> = {
  ...
  "imposter-syndrome-shader": [["className", "string", "—", "Optional wrapper classes."]],
}

function PropList({ slug }: { slug: ComponentSlug }) {
  const rows = propRows[slug] ?? []
  return <section className="mt-8"><h2 className="text-lg font-semibold">Props</h2><div className="mt-3 overflow-hidden rounded-lg border"><div className="grid grid-cols-[minmax(7rem,1fr)_minmax(7rem,1fr)_5rem] gap-3 border-b bg-muted/30 px-4 py-2 text-xs font-medium text-muted-foreground"><span>Name</span><span>Type</span><span>Default</span></div>{rows.map(([name, type, defaultValue, description]) => <div key={name} className="grid grid-cols-[minmax(7rem,1fr)_minmax(7rem,1fr)_5rem] gap-3 border-b px-4 py-3 text-xs last:border-b-0"><div><code className="font-medium text-foreground">{name}</code><p className="mt-1 text-muted-foreground md:hidden">{description}</p></div><code className="break-words text-muted-foreground">{type}</code><code className="text-muted-foreground">{defaultValue}</code></div>)}</div></section>
}
...
export function ComponentCustomizer({ slug }: { slug: ComponentSlug }) {
  ...
  const [colors, setColors] = useState(slug === "caustics-shader-background" ? ["#020617", "#075985", "#22d3ee", "#ecfeff"] : ["#000000", "#00FF37", "#B6FF41", "#000000"])
  ...
  if (slug === "imposter-syndrome-shader") return <div className="mt-4"><div className="rounded-lg border bg-muted/20 p-3 sm:p-6"><ImposterSyndromeShader /></div><section className="mt-8"><h2 className="text-lg font-semibold">Customize</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">Open the preview to interact with the original Shadertoy shader and drag the sun across the sky.</p></section><PropList slug={slug} /></div>
  const Shader = slug === "caustics-shader-background" ? CausticsShaderBackground : SmokeShaderBackground
  const isCaustics = slug === "caustics-shader-background"
